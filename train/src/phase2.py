...
from utils.save_model import calculate_nc, save_watermark_image, save_adaptor
...
from utils.wm_transform import WatermarkAugmenter
from wm_model.adaptor0 import Adaptor
...
def train(args):
    ...
    # ================================load WMencoder adapter and Noise layer=====================================#
    wm_checkpoint = torch.load(args.wm_model_path, map_location="cpu")
    ...
    wm_encoder = wm_encoder.to(device)
    ...
    adaptor_net = Adaptor(4, 64)
    adaptor_net.to(device)

    # ==============================Frozen vae_encoder and wm_encoder=====================================#
    ...
    for name, param in wm_encoder.named_parameters():
        param.requires_grad_(False)

    # =======================================get trainable params========================================#
    vae_params = [p for p in vae.parameters() if p.requires_grad]
    wm_params = list(wm_decoder.parameters())
    adaptor_params = list(adaptor_net.parameters())
    print(f"Total params: {sum(p.numel() for p in vae_params + wm_params + adaptor_params)}")

    optimizer = optim.Adam([
        {'params': adaptor_params, 'lr': args.learning_rate},
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (images, filenames) in enumerate(progress_bar):
            ...
            alpha = adaptor_net(latents[:, 0:4, :, :], watermark_latent)
            latents_wm[:, 0:4, :, :] = latents[:, 0:4, :, :] + alpha * watermark_latent
            ...
            if args.clip_grad > 0:
                torch.nn.utils.clip_grad_norm_(adaptor_params + wm_params, args.clip_grad)
            ...
            current_alpha = alpha.mean().item()
            alpha_sum += current_alpha
            avg_alpha = alpha_sum / (batch_idx + 1)

            progress_bar.set_postfix({"loss": avg_loss, "loss_img": avg_loss_img, "nc_value": nc_value, "alpha": avg_alpha})
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_adaptor(wm_encoder, wm_decoder, adaptor_net, optimizer, epoch + 1, avg_loss, args, is_best=True)

        if args.save_every_epoch:
            save_adaptor(wm_encoder, wm_decoder, adaptor_net, optimizer, epoch + 1, avg_loss, args)

        save_adaptor(wm_encoder, wm_decoder, adaptor_net, optimizer, args.epochs, avg_loss, args, is_final=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="fintune wm_decoder and train adapter")
    parser.add_argument("--data_dir", type=str, default=r'train/dataset/train')
    parser.add_argument("--output_dir", type=str, default="train/result/stage2")
    ...
    parser.add_argument("--wm_model_path", type=str, default="train/result/stage2/wm_encoder_decoder_final.ckpt")
