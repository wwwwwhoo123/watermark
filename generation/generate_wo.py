...
from train.src.vine.load_finetune_mode import load_finetuned_decoder, load_finetuned_models
import json
...
def main():
    ...
    with open('./example/config/config_list.yaml', 'r') as file:
        cfgs = yaml.safe_load(file)

    vae, wm_encoder, wm_decoder = load_finetuned_models(cfgs['model_id'], cfgs['vae_path'], cfgs['wm_path'])
    ...
    csv_header = ['Image Name', 'Enhance PSNR', 'Enhance SSIM', 'NC']
    ...
    for (image_id, text_embeddings, negative_prompt_embeds,
        ...
        with torch.no_grad():
            ...
            latent[:, 0:4, :, :] = latent[:, 0:4, :, :] + watermark_latent
        ...
        with open(csv_file_path, 'a', newline='') as csvfile:
            csv.writer(csvfile).writerow([image_id, psnr_value, ssim_value, nc_value])
