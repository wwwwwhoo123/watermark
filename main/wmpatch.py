...
class GTWatermark():
    ...
    #         tmp_mask = torch.tensor(self._circle_mask(gt_init.shape[-1], r=i)).to(
    #             self.device)  # circle mask in bool value
    #         gt_patch[:, self.w_channel, tmp_mask] = gt_patch[
    #             0, self.w_channel, 0, i].item()  # set the value inside the circle to be a value from Gaussian Distribution
    #     return gt_patch
    #
    # def _get_watermarking_mask(self, gt_patch):
    #     watermarking_mask = torch.zeros(gt_patch.shape, dtype=torch.bool).to(self.device)
    #     watermarking_mask[:, self.w_channel] = torch.tensor(self._circle_mask(gt_patch.shape[-1], r=self.w_radius)).to(
    #         self.device)
    ...
    #
    #     reversed_latents_w_fft = torch.fft.fftshift(torch.fft.fft2(latents), dim=(-1, -2))[
    #         self.watermarking_mask].flatten()
...
#                  w_settings={0: [1, 5, 9], 1: [2, 6, 10], 2: [3, 7], 3: [4, 8]}, generator=None):
#         self.device = device
#         # from latent tensor
#         self.shape = shape
#         self.dtype = dtype
...
#                 tmp_mask_alter, tmp_mask_inner = self._circle_mask(gt_init.shape[-1], r=w_radius), self._circle_mask(
#                     gt_init.shape[-1], r=w_radius - 1)
