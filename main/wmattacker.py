...
from compressai.compressai.zoo import bmshj2018_factorized, bmshj2018_hyperprior, mbt2018_mean, mbt2018, \
    cheng2020_anchor
...
class JPEGAttacker(WMAttacker):
    def __init__(self, quality=80):
        self.jpeg = Jpeg(quality)

    def save_noised_image(self, tensor_img, out_path):
        img = (tensor_img.squeeze(0) * 0.5 + 0.5).clamp(0, 1)
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in zip(image_paths, out_paths):
            ...
            noised_img = self.jpeg(img)
            # noised_img = (noised_img / 2 + 0.5).clamp(0, 1)
            # noised_img = noised_img.cpu().permute(0, 2, 3, 1).float().numpy()
            # noised_img.save(out_path)
            self.save_noised_image(noised_img, out_path)
...
class CropAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in zip(image_paths, out_paths):
            ...
            h_start, h_end, w_start, w_end = get_random_rectangle_inside((w, h), self.crop_size, self.crop_size)
            # Create a mask with the same size as the original image
            mask = Image.new('L', (w, h), 0)
            mask.paste(255, (w_start, h_start, w_end, h_end))
            # Apply the mask to the original image
            img_masked = ImageChops.multiply(img.convert('RGB'), mask.convert('RGB'))
